...
class Image:
	...
	def add_indirection_table(self,conf):
		...
		for lib in conf:
			...
			self.libInInd[lib[0]] = [int(addr,16)+byteCount,0,int(addr,16)+byteCount+padding]
			if lib[0] not in self.libList:
				#Padding
				code += b'\x90' * padding
				byteCount += padding
			else:
			...
			self.libInInd[lib[0]][1] = int(addr,16)+byteCount
			#.ind.lib last instruction address
			code += b'\x90'*(self.libInInd[lib[0]][2] - (int(addr,16)+byteCount))
			byteCount += self.libInInd[lib[0]][2] - (int(addr,16)+byteCount)
...
if __name__ == '__main__':
	...
	img = Image(elfFile,extract_libs(params.obj,True))
