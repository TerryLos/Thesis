...
class Image:
	...
	def correct_calls(self,disass):
		...
		for inst in instList:
			tmp , rangeLib = self._find_lib(inst.address)
			patch = b'\xc7\xc0'
			...
			elif jump_mnemonic(inst.mnemonic) and DEBUG and (funcInfo is not None and rangeLib is not None ) \
			...
			elif varInfo and not jump_mnemonic(inst.mnemonic) and False:
				for var in varInfo:
	...
	def add_indirection_table(self):
		...
		if self.baseAddr+byteCount > self.baseAddr+byteCount+padding:
			print("[Error] - The space reserved for lib"+lib+" is too small, abort.")
			sys.exit()
